"""
Compatibility Data Processor for Earthquake Alert System
Processes earthquake data using pandas and scikit-learn to generate the same output format
as the PySpark processor, enabling the system to work without PySpark/Java compatibility issues.
"""
...
def load_data(file_path):
    ...
    print("\n" + "=" * 80)
    print("EARTHQUAKE ALERT SYSTEM - Compatibility Data Processor")
    print("=" * 80)
...
def save_results(df, output_dir):
    """Save processed data in the same format as the PySpark processor"""
    print("\nSaving Results...")
    
    os.makedirs(output_dir, exist_ok=True)
    
    # Select output columns to match PySpark processor output
    output_cols = [
    ...
    # Save CSV (matching the expected format)
    csv_path = os.path.join(output_dir, 'earthquake_alerts.csv')
    
    # Handle permission issues by trying alternative filename
    try:
        output_df.to_csv(csv_path, index=False)
        print(f"   CSV saved to: {csv_path}")
    except PermissionError:
        alt_csv_path = os.path.join(output_dir, 'earthquake_alerts_new.csv')
        output_df.to_csv(alt_csv_path, index=False)
        print(f"   ⚠️  CSV saved to alternative location: {alt_csv_path}")
        print(f"   ⚠️  Please close any applications using the original file and rename if needed")
